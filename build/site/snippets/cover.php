...
<figure class="inline-block lg:ml-12 shadow-cover rounded-lg">
    <img class="rounded-t-lg" src="<?= $thumb->url() ?>" title="<?= $image->titleAttribute()->html() ?><?php e($image->source()->isNotEmpty(), ' - ' . $image->source()->html()) ?>" alt="<?= $image->altAttribute()->html() ?>" width="<?= $thumb->width() ?>" height="<?= $thumb->height() ?>">
