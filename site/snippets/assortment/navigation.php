    ...
    <div class="w-1/2 xl:w-1/3 flex justify-center text-shadow leading-none">
        <figure class="group mx-2 md:mx-4 mb-4 md:mb-8 lg:mb-12 inline-block relative<?php e($category->isOpen(), ' cursor-default') ?>"<?php e($category->isOpen(), ' title="' . t('Du bist hier') . '"') ?>">
            <?php e(!$category->isOpen(), '<a href="' . $category->url() . '">') ?>
                ...
                <?php endif ?>
                <?= $category->getCover()->createImage('rounded-lg shadow-cover', 'assortment.navigation', true) ?>
                <figcaption class="py-1 xs:py-2 left-0 right-0 sketch text-lg xs:text-2xl md:text-5xl text-white text-center<?php e($category->isOpen(), ' bg-orange-medium ',  ' bg-red-light group-hover:bg-red-medium ') ?>absolute select-none transition-all" style="bottom: 0.75rem"><?= $category->title()->html() ?></figcaption>
